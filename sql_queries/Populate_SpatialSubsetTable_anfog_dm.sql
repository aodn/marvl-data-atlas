...
(width_bucket(d."DEPTH", -2.5, 502.5, 101)-1)*5, --we are binning from surface to 500m limit every 5m
max(replace(replace(d."DEPTH_quality_control", '8', '2'), '9', '')),
avg(d."TEMP"),
...
GROUP BY s.source_id, d.file_id, date_trunc('minute', d."TIME" AT TIME ZONE 'UTC'), width_bucket(d."DEPTH", -2.5, 502.5, 101)
ORDER BY d.file_id, date_trunc('minute', d."TIME" AT TIME ZONE 'UTC'), width_bucket(d."DEPTH", -2.5, 502.5, 101);
