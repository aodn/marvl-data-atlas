...
date_trunc('month', s."TIME" AT TIME ZONE 'UTC') + interval '14 days' AS "TIME", -- we consider TIME buckets monthly with first value centred on 2007-01-15 : [2007-01-01;2007-01-31[, [2007-02-01;2007-02-28[, etc...
...
(width_bucket(CASE WHEN s."DEPTH_QC" IN ('0', '1', '2') THEN s."DEPTH" ELSE s."NOMINAL_DEPTH" END, -5, 505, 51)-1)*10 AS "DEPTH", -- we consider DEPTH buckets of size 10 with first value centred on 0 : [-5;5[, [5;15[, etc... If field DEPTH is NULL, NOMINAL_DEPTH is considered.
