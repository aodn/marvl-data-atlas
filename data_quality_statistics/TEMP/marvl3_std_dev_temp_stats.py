...
@author: lbesnard
"""
# script to look for std temp > std_dev (4 by default) and retrieve the source data. The source data is copied into a 'bin' directory
...
# find rows in marvl3.data_atlas WHERE std dev of TEMP is greater than std_dev defined above
def get_temp_stddev(cur,std_dev):
    sql_query_find_temp_std = """SELECT "LONGITUDE_bin","LATITUDE_bin","TIME_bin","DEPTH_bin","TEMP_stddev" FROM marvl3.data_atlas WHERE "TEMP_stddev">"""+ str(std_dev) +""" ORDER BY  "TEMP_stddev" DESC"""
    cur.execute(sql_query_find_temp_std)
...
def save_sql_query_temp_csv(folder_path,temp_stddev):
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    # save header to file
    f = open(folder_path + os.path.sep + 'rows.csv','w')
    f.write( 'LON_bin,LAT_bin,TIME_bin,DEPTH_bin,TEMP_STDDEV\n')
    f.close()

    # save data
    f_handle = file(folder_path + os.path.sep + 'rows.csv', 'a')
    np.savetxt(f_handle, temp_stddev,delimiter=",", fmt='%s')
    f_handle.close()

def main():
    cur           = connect(db_dbname,db_user,db_host)
    temp_stddev_4 = get_temp_stddev(cur,std_dev)
    save_sql_query_temp_csv(folder_path,temp_stddev_4)
    global std_dev_bin

    i_row = 0
    for row in temp_stddev_4:
...
std_dev     = 4
folder_path = os.getenv("HOME") + os.path.sep  +'IMOS/marvl3_statistics/TEMP'
